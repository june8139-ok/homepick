import ReservationCard from "./ReservationCard";
import type {
    Apartment,
    SubscriptionSchedule,
  } from "../../types/apartment";
  
  type ApplyHomeData = Record<string, unknown>;
  
  function getText(
    data: ApplyHomeData | undefined,
    key: string
  ) {
    const value = data?.[key];
  
    if (
      value === null ||
      value === undefined ||
      value === ""
    ) {
      return "";
    }
  
    return String(value).trim();
  }
  
  function formatDate(value?: string | null) {
    if (!value) return "일정 확인 중";
  
    const normalized = value
      .replace(/\./g, "-")
      .replace(/\//g, "-");
  
    const parts = normalized.split("-");
  
    if (parts.length !== 3) {
      return value;
    }
  
    return `${parts[0]}.${parts[1]}.${parts[2]}`;
  }
  
  function formatPeriod(
    start?: string | null,
    end?: string | null
  ) {
    if (!start && !end) {
      return "일정 확인 중";
    }
  
    if (start && end && start !== end) {
      return `${formatDate(start)} ~ ${formatDate(end)}`;
    }
  
    return formatDate(start ?? end);
  }
  
  function getTimelineItems(
    schedule?: SubscriptionSchedule
  ) {
    return [
      {
        label: "모집공고",
        value: formatDate(
          schedule?.announcementDate
        ),
      },
      {
        label: "특별공급",
        value: formatPeriod(
          schedule?.specialSupplyStartDate,
          schedule?.specialSupplyEndDate
        ),
      },
      {
        label: "1순위",
        value: formatPeriod(
          schedule?.firstPriorityStartDate,
          schedule?.firstPriorityEndDate
        ),
      },
      {
        label: "2순위",
        value: formatPeriod(
          schedule?.secondPriorityStartDate,
          schedule?.secondPriorityEndDate
        ),
      },
      {
        label: "당첨자 발표",
        value: formatDate(
          schedule?.winnerDate
        ),
      },
      {
        label: "계약기간",
        value: formatPeriod(
          schedule?.contractStartDate,
          schedule?.contractEndDate
        ),
      },
    ];
  }
  
  function getStatusDescription(status: string) {
    switch (status) {
      case "청약예정":
        return "청약 접수를 앞두고 있는 단지입니다.";
  
      case "청약중":
        return "현재 청약 접수가 진행 중입니다.";
  
      case "당첨자발표":
        return "당첨자 발표 및 서류 일정을 확인할 단계입니다.";
  
      case "계약중":
        return "당첨자 계약 일정이 진행 중입니다.";
  
      case "청약마감":
        return "청약과 계약 일정이 종료된 단지입니다.";
  
      default:
        return "청약 일정과 모집공고를 확인해보세요.";
    }
  }
  
  function InfoItem({
    label,
    value,
  }: {
    label: string;
    value: string;
  }) {
    return (
      <div className="rounded-2xl bg-zinc-50 p-4">
        <p className="text-xs font-semibold text-zinc-500">
          {label}
        </p>
  
        <p className="mt-2 break-words text-sm font-bold leading-6 text-zinc-900">
          {value || "정보 확인 중"}
        </p>
      </div>
    );
  }
  
  export default function SubscriptionDetail({
    apartment,
  }: {
    apartment: Apartment;
  }) {
    const applyHome =
      apartment.applyHome as ApplyHomeData | undefined;
  
    const schedule = apartment.subscription;
    const timelineItems =
      getTimelineItems(schedule);
  
    const totalSupply =
      apartment.totalSupply ??
      Number(
        getText(
          applyHome,
          "TOT_SUPLY_HSHLDCO"
        )
      );
  
    const developer =
      apartment.projectInfo?.developer ||
      getText(applyHome, "BSNS_MBY_NM");
  
    const builder =
      apartment.builder ||
      getText(
        applyHome,
        "CNSTRCT_ENTRPS_NM"
      );
  
    const phone =
      apartment.projectInfo?.phone ||
      getText(applyHome, "MDHS_TELNO");
  
    const moveInDate =
      apartment.projectInfo?.moveInDate ||
      getText(
        applyHome,
        "MVN_PREARNGE_YM"
      );
  
    const housingType =
      getText(
        applyHome,
        "HOUSE_DTL_SECD_NM"
      ) ||
      apartment.type;
  
    const applyHomeUrl =
      apartment.applyHomeUrl ||
      schedule?.applyHomeUrl ||
      getText(applyHome, "PBLANC_URL");
  
    return (
      <div className="mt-6 space-y-6">
        {/* 청약 상태 요약 */}
        <section className="rounded-3xl border border-blue-100 bg-gradient-to-br from-blue-50 to-white p-7 shadow-sm">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm font-bold text-blue-600">
                SUBSCRIPTION STATUS
              </p>
  
              <h2 className="mt-2 text-2xl font-extrabold">
                {apartment.status}
              </h2>
  
              <p className="mt-2 text-sm leading-6 text-zinc-600">
                {getStatusDescription(
                  apartment.status
                )}
              </p>
            </div>
  
            <div className="grid grid-cols-2 gap-3 sm:min-w-[320px]">
              <InfoItem
                label="공급 세대수"
                value={
                  Number.isFinite(totalSupply) &&
                  totalSupply > 0
                    ? `${totalSupply.toLocaleString()}세대`
                    : "정보 확인 중"
                }
              />
  
              <InfoItem
                label="당첨자 발표"
                value={formatDate(
                  schedule?.winnerDate
                )}
              />
            </div>
          </div>
        </section>
  
        {/* 청약 일정 */}
        <section className="rounded-3xl border border-zinc-200 bg-white p-7 shadow-sm">
          <div>
            <p className="text-sm font-bold text-blue-600">
              SUBSCRIPTION SCHEDULE
            </p>
  
            <h2 className="mt-1 text-2xl font-extrabold">
              청약 일정
            </h2>
  
            <p className="mt-2 text-sm text-zinc-500">
              모집공고부터 계약기간까지 주요 일정을
              확인하세요.
            </p>
          </div>
  
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {timelineItems.map(
              (item, index) => (
                <div
                  key={item.label}
                  className="relative rounded-2xl border border-zinc-200 bg-zinc-50 p-5"
                >
                  <div className="flex items-center gap-3">
                    <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-600 text-xs font-extrabold text-white">
                      {index + 1}
                    </span>
  
                    <p className="font-bold">
                      {item.label}
                    </p>
                  </div>
  
                  <p className="mt-4 text-sm font-semibold text-zinc-700">
                    {item.value}
                  </p>
                </div>
              )
            )}
          </div>
        </section>
  
        {/* 가격 및 공급정보 */}
        <section className="rounded-3xl border border-zinc-200 bg-white p-7 shadow-sm">
          <div>
            <p className="text-sm font-bold text-emerald-600">
              PRICE & SUPPLY
            </p>
  
            <h2 className="mt-1 text-2xl font-extrabold">
              가격 및 공급정보
            </h2>
          </div>
  
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <InfoItem
              label="공급 세대수"
              value={
                Number.isFinite(totalSupply) &&
                totalSupply > 0
                  ? `${totalSupply.toLocaleString()}세대`
                  : "정보 확인 중"
              }
            />
  
            <InfoItem
              label="분양가"
              value={
                apartment.price ||
                apartment.priceDetail
                  ?.salePrice ||
                "주택형별 분양가 연동 준비 중"
              }
            />
  
            <InfoItem
              label="계약금"
              value={
                apartment.priceDetail
                  ?.contractPrice ||
                "모집공고 확인"
              }
            />
  
            <InfoItem
              label="중도금"
              value={
                apartment.priceDetail
                  ?.middlePayment ||
                "모집공고 확인"
              }
            />
          </div>
  
          <div className="mt-4 rounded-2xl bg-blue-50 p-5">
            <p className="text-sm font-bold text-blue-700">
              주택형별 공급정보
            </p>
  
            <p className="mt-2 text-sm leading-6 text-blue-900/70">
              다음 단계에서 청약홈 주택형 상세 API를
              연결하면 타입별 세대수와 분양가가 이
              영역에 자동으로 표시됩니다.
            </p>
          </div>
        </section>
  
        {/* 사업개요 */}
        <section className="rounded-3xl border border-zinc-200 bg-white p-7 shadow-sm">
          <div>
            <p className="text-sm font-bold text-zinc-500">
              PROJECT OVERVIEW
            </p>
  
            <h2 className="mt-1 text-2xl font-extrabold">
              사업개요
            </h2>
          </div>
  
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <InfoItem
              label="사업지 주소"
              value={apartment.region}
            />
  
            <InfoItem
              label="주택 구분"
              value={housingType}
            />
  
            <InfoItem
              label="시공사"
              value={builder}
            />
  
            <InfoItem
              label="사업주체"
              value={developer}
            />
  
            <InfoItem
              label="입주 예정"
              value={
                moveInDate || "정보 확인 중"
              }
            />
  
            <InfoItem
              label="문의전화"
              value={phone}
            />
  
            {apartment.projectInfo
              ?.parking && (
              <InfoItem
                label="주차대수"
                value={
                  apartment.projectInfo.parking
                }
              />
            )}
  
            {apartment.projectInfo
              ?.scale && (
              <InfoItem
                label="사업 규모"
                value={
                  apartment.projectInfo.scale
                }
              />
            )}
  
            {apartment.projectInfo
              ?.usage && (
              <InfoItem
                label="용도"
                value={
                  apartment.projectInfo.usage
                }
              />
            )}
          </div>
        </section>
        <ReservationCard
          apartmentSlug={apartment.slug}
          apartmentName={apartment.name}
          mode="subscription"
          phoneNumber={phone}
          floorPlanNames={
            apartment.images.floorPlans
              .map((item) => item.name)
              .filter(Boolean)
         }
       />
        {/* 청약홈 공식 연결 */}
        <section className="rounded-3xl bg-zinc-900 p-7 text-white shadow-sm">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm font-bold text-blue-300">
                APPLYHOME
              </p>
  
              <h2 className="mt-1 text-2xl font-extrabold">
                공식 모집공고 확인
              </h2>
  
              <p className="mt-2 text-sm leading-6 text-zinc-300">
                청약 신청과 최종 자격 확인은 청약홈
                공식 페이지에서 진행해주세요.
              </p>
            </div>
  
            {applyHomeUrl ? (
              <a
                href={applyHomeUrl}
                target="_blank"
                rel="noreferrer"
                className="inline-flex min-h-12 items-center justify-center rounded-2xl bg-blue-500 px-6 py-3 text-sm font-extrabold text-white transition hover:bg-blue-400"
              >
                청약홈에서 공식 확인하기 ↗
              </a>
            ) : (
              <span className="inline-flex min-h-12 items-center justify-center rounded-2xl bg-zinc-700 px-6 py-3 text-sm font-bold text-zinc-300">
                청약홈 링크 준비 중
              </span>
            )}
          </div>
        </section>
      </div>
    );
  }